...
var ExAsUser = (function() {
    ...
    var tb = new DataTable(idTable, {
        ...
        columnDefs: [{
            ...
            },
            {
                ...
                data: null,
                render: function(data, type, full, meta) {
                    return `${data.departure_date != null ? data.departure_date + ", " : ""} ${data.departure_time}`;
                },
            },
            {
                targets: i++,
    ...
    var Transaction = function() {
        showHideOptions($('#type_schedule_bus').val());
        // inputEmailTrigger();
        addTrigger();
    ...
    var updateClickTrigger = function() {
        $("table tbody").on("click", ".tombolEdit", function() {
            ...
            $('#edit_departure_day').val(drop.departure_day);
            ...
            ExAl.Toast.Delete({}, function(result) {
                if (result) {
                    $.ajax({
                        ...
                        async: false,
                        data: {
                            schedule_number: drop.schedule_number,
                            scrty: true
                        },
                        success: function(response) {
                            if (ExAs.Utils.Json.valid(response)) {
    ...
    var showHideOptions = function(value) {
        $(".prepend_schedule_number").html(value)
        if (value != 'HOLIDAY') {
    ...
    $('#type_schedule_bus').on('change', function() {
        showHideOptions(this.value);
    });

    $('#edit_type_schedule_bus').on('change', function() {
        showHideOptions(this.value);
    });
