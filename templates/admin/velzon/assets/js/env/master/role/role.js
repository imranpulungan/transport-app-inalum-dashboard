...
var ExAsUser = (function() {
                ...
                if (ExAs.Utils.Json.valid(respon)) {
                    ...
                    if (res.success) {
                        ...
                        $.each(res.data, function(i, item) {
                            var isReady = availableData.includes(item.id_role);
                            if (isReady === false) {
                                totalDataExist++;
                                tb.row.add([
                                    totalDataExist,
    ...
    var updateClickTrigger = function() {
        $("table tbody").on("click", ".tombolEdit", function() {
            ...
            $('#id_role_edit').val(drop[3])
            $('#nm_role_edit').val(drop[1])
