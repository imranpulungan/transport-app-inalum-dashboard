...
var ExAsUser = (function() {
                        ...
                        $.each(res.data, function(i, item) {
                            ...
                            if (isReady === false) {
                                ...
                                tb.rows().every(function(rowIdx, tableLoop, rowLoop) {
                                    var data = this.data();
                                    if (data[5] == item.id) {
                                        if (data[1] !== item.no_dokumen) {
                                            tb.cell(rowIdx, 1)
                                                .data(item.no_dokumen)
                                        }

                                        if (data[2] !== item.nm_jenis) {
                                            tb.cell(rowIdx, 2)
                                                .data(item.nm_jenis)
                                        }

                                        if (data[3] !== item.judul) {
                                            tb.cell(rowIdx, 3)
                                                .data(item.judul)
                                        }
    ...
    var updateClickTrigger = function() {
        $("table tbody").on("click", ".tombolEdit", function() {
            ...
            $('#id_jenis_edit').val(drop[5])
            $('#no_dokumen_edit').val(drop[1])
            $('#nm_jenis_edit').val(drop[2])
            $('#judul_edit').val(drop[3])
